...
/**
 * True when an error looks like a transport/connectivity problem rather
 * than a real auth rejection. Used so a host that boots while the
 * internet is down does NOT wipe an otherwise-valid cloud session (which
 * would force a needless re-login once connectivity returns).
 */
function isTransientCloudError(e: any): boolean {
...
async function bootstrapCloudSession() {
  try {
    ...
    const [staffRow, adminRow, legacyRow] = await Promise.all([
      prisma.syncState
        .findUnique({ where: { key: CLOUD_SESSION_STAFF_KEY } })
        .catch(() => null),
      prisma.syncState
        .findUnique({ where: { key: CLOUD_SESSION_ADMIN_KEY } })
        .catch(() => null),
      prisma.syncState
        .findUnique({ where: { key: CLOUD_SESSION_KEY } })
        .catch(() => null),
    ]);

    const staffSaved =
      (staffRow?.valueJson as any) || (legacyRow?.valueJson as any) || null;
    ...
    const validate = async (s: CloudSession, key: string) => {
      try {
        const me = await cloudJson<any>('GET', '/auth/me', undefined, {
          requireAuth: true,
          senderId: 0,
          businessCodeHint: s.businessCode,
        });
        ...
        await persistCloudSession(key, {
          token: refreshed.token,
          businessCode: refreshed.businessCode,
          role: refreshed.role,
          userId: refreshed.userId,
        });
      } catch (e) {
        // Offline at boot: keep the saved session (both in memory and on
        // disk) so the app stays usable and re-validates later. Only a
        // genuine auth rejection (cloudJson already cleared it on 401/403)
        // should drop the session here.
        if (isTransientCloudError(e)) return;
    ...
    await Promise.all([
      staffSession
        ? validate(staffSession, CLOUD_SESSION_STAFF_KEY)
        : Promise.resolve(),
      adminSession
        ? validate(adminSession, CLOUD_SESSION_ADMIN_KEY)
        : Promise.resolve(),
    ]);
  } catch {
    // ignore
...
export function setCloudToken(
  ...
  if (!next || !businessCode) {
    staffSession = null;
    void persistCloudSession(CLOUD_SESSION_STAFF_KEY, {
      token: null,
      businessCode: null,
      role: null,
      userId: null,
...
export function setCloudSession(next: {
  token: string;
  businessCode: string;
  role: 'ADMIN' | 'CASHIER' | 'WAITER';
  userId: number;
  ...
  void persistCloudSession(pickStoreKey(next.role), {
    token: s.token,
...
export function hasCloudSessionForSender(
  senderId: number,
  businessCode: string,
): boolean {
...
export function isCloudAdminForSender(
  senderId: number,
  businessCode: string,
): boolean {
  const s = perSender.get(senderId);
  return Boolean(
    s && s.businessCode === businessCode && s.role === 'ADMIN' && s.token,
  );
}

export function clearCloudAdminSession() {
  adminSession = null;
  void persistCloudSession(CLOUD_SESSION_ADMIN_KEY, {
    token: null,
    businessCode: null,
    role: null,
    userId: null,
...
export function clearCloudSession() {
  ...
  void persistCloudSession(CLOUD_SESSION_KEY, {
    token: null,
    businessCode: null,
    role: null,
    userId: null,
...
export function hasCloudSession(businessCode: string): boolean {
  return Boolean(
    (staffSession &&
      staffSession.businessCode === businessCode &&
      staffSession.token) ||
      (adminSession &&
        adminSession.businessCode === businessCode &&
        adminSession.token),
  );
}
...
export async function getCloudToken(): Promise<string | null> {
  ...
  if (
    adminSession &&
    adminSession.businessCode === cfg.businessCode &&
    adminSession.token
  )
    return adminSession.token;
  if (
    staffSession &&
    staffSession.businessCode === cfg.businessCode &&
    staffSession.token
  )
...
export async function cloudJson<T = any>(
  ...
  const usedSenderToken = Boolean(
    senderSession &&
      senderSession.businessCode === cfg.businessCode &&
      senderSession.token,
  );
  ...
  const p = String(path || '');
  const wantsAdmin = p.startsWith('/admin') || p.startsWith('/auth/users');
  // IMPORTANT:
  ...
  const senderIsAdmin = Boolean(
    senderSession && senderSession.role === 'ADMIN',
  );
  const picked: CloudSession | null = wantsAdmin
    ? usedSenderToken && senderIsAdmin
      ? senderSession!
      : adminSession || null
  ...
  if (picked && picked.businessCode === cfg.businessCode && picked.token) {
    ...
    throw new Error(
      wantsAdmin ? 'Admin login required' : 'Not logged in to cloud backend',
    );
  }
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (Array.isArray(issues) && issues.length) {
        const preview = issues
          ...
          .join('; ');
        if (preview)
          msg = `${String(msg)} (${preview}${issues.length > 5 ? '…' : ''})`;
      }
    } catch {
      // ignore
    }
    if (res.status === 401 || res.status === 403) {
      ...
      if (
        usedSenderToken &&
        senderId &&
        picked?.token === senderSession?.token
      ) {
