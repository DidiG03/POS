...
ipcMain.handle('tickets:print', async (_e, input) => {
  ...
  const ok = await sendToPrinter(ip, port, data);
  // Optionally, enqueue a PrintJob record
  try {
...
ipcMain.handle('tables:setOpen', async (_e, input) => {
  ...
  if (open) atMap[kKey] = new Date().toISOString(); else delete atMap[kKey];
...
ipcMain.handle('menu:syncFromUrl', async (_e, raw) => {
  ...
  const url = input.url || process.env.MENU_API_URL || 'https://ullishtja-agroturizem.com/api/pos-menu?lang=sq';
  ...
  for (const cat of categories) {
    ...
    for (const it of items) {
      ...
      const isKg: boolean = Boolean((it?.tags && it.tags.isKg) || it?.isKg);
      if (isKg) isKgSkus.add(sku);
  ...
  await prisma.syncState.upsert({ where: { key: 'menu:isKgSkus' }, create: { key: 'menu:isKgSkus', valueJson: Array.from(isKgSkus) }, update: { valueJson: Array.from(isKgSkus) } });
  return { categories: catCount, items: itemCount };
});

ipcMain.handle('menu:listCategoriesWithItems', async () => {
  const [cats, isKgRow] = await Promise.all([
    prisma.category.findMany({
      where: { active: true },
      orderBy: { sortOrder: 'asc' },
      include: { items: { where: { active: true }, orderBy: { name: 'asc' } } },
    }),
    prisma.syncState.findUnique({ where: { key: 'menu:isKgSkus' } }),
  ]);
  let isKgSet = new Set<string>(((isKgRow?.valueJson as any) || []) as string[]);
  // Fallback: if we don't have the mapping yet, fetch upstream once to infer isKg
  if (isKgSet.size === 0) {
    try {
      const url = process.env.MENU_API_URL || 'https://ullishtja-agroturizem.com/api/pos-menu?lang=sq';
      const r = await fetch(url, { headers: { Accept: 'application/json, text/plain;q=0.9, */*;q=0.8' } as any } as any);
      const ct = r.headers.get('content-type') || '';
      const body: any = ct.includes('application/json') ? await r.json() : await r.text().then((txt) => {
        const s = txt.indexOf('{'); const e = txt.lastIndexOf('}'); return JSON.parse(txt.slice(s, e + 1));
      });
      const categories = Array.isArray(body?.data)
        ? body.data
        : Array.isArray(body?.categories)
      ...
      for (const c of categories) {
        ...
        for (const it of items) {
          const sku = it?.sku ?? it?.id; if (!sku) continue;
          const isKg = Boolean((it?.tags && it.tags.isKg) || it?.isKg);
          if (isKg) skus.push(String(sku));
        }
      }
      isKgSet = new Set<string>(skus);
      await prisma.syncState.upsert({ where: { key: 'menu:isKgSkus' }, create: { key: 'menu:isKgSkus', valueJson: Array.from(isKgSet) }, update: { valueJson: Array.from(isKgSet) } });
    } catch {}
  }
