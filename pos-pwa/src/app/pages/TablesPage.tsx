...
export default function TablesPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const saved = await api.layout.get(user.id, area);
          ...
          {editable && (
            <button
              ...
              onClick={async () => {
                ...
                await api.layout.save(user.id, area, nodes);
                setEditable(false);
              }}
            >
              Save
      ...
      <div className="relative w-full h-[520px] rounded bg-gray-800 overflow-hidden">
        ...
        {nodes?.map((t, idx) => (
          <DraggableCircle
            ...
            onMove={(_x, _y) =>
              setNodes((prev) =>
                prev?.map((n, i) => (i === idx ? { ...n, x: _x, y: _y } : n)) ?? prev,
              )
            }
            onClick={() => {
              if (editable) return;
              ...
              if (isOpen(area, t.label)) {
                (async () => {
                  const data = await api.tickets.getLatestForTable(area, t.label);
...
function DraggableCircle({ node, editable, onMove, onClick, colorClass }: { node: TableNode; editable: boolean; onMove: (_x: number, _y: number) => void; onClick?: () => void; colorClass?: string }) {
