...
export default function AdminLayout() {
  ...
  useEffect(() => {
    (async () => {
      const unread = await api.admin.listNotifications({ onlyUnread: true }).catch(() => []);
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {unreadCount > 0 && (
                      <button
                        ...
                        onClick={async () => {
                          await api.admin.markAllNotificationsRead().catch(() => {});
                          setUnreadCount(0);
                        }}
                      >
                        Mark all as read
...
function AdminNotificationsList({ onCount }: { onCount: (n: number) => void }) {
  ...
  useEffect(() => {
    (async () => {
      const all = await api.admin.listNotifications({}).catch(() => []);
      setItems(all);
      const unread = await api.admin.listNotifications({ onlyUnread: true }).catch(() => []);
